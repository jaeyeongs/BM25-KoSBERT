...
class LMOrderedIterator(object):
    def __init__(self, data, bsz, bptt, device="cpu", ext_len=None):
        ...
        # Work out how cleanly we can divide the KorQuADDataset into bsz parts.
...
def get_lm_corpus(datadir, dataset):
    ...
    if os.path.exists(fn):
        logger.info("Loading cached KorQuADDataset...")
        corpus = torch.load(fn_pickle)
    elif os.path.exists(fn):
        logger.info("Loading cached KorQuADDataset from pickle...")
        with open(fn, "rb") as fp:
            corpus = pickle.load(fp)
    else:
        logger.info("Producing KorQuADDataset {}...".format(dataset))
        kwargs = {}
